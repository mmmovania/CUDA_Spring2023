...
int main(int argc, char **argv) {
    ...
    cl_mem a_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                      m * p * sizeof(float), NULL, &err);
    cl_mem b_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                      p * n * sizeof(float), NULL, &err);
    ...
    err = clEnqueueWriteBuffer(queue, a_mem_obj, CL_TRUE, 0,
                               m * p * sizeof(float), A, 0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, b_mem_obj, CL_TRUE, 0,
                                p * n * sizeof(float), B, 0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, m_mem_obj, CL_TRUE, 0, sizeof(int), &m,
                                0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, p_mem_obj, CL_TRUE, 0, sizeof(int), &p,
                                0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, n_mem_obj, CL_TRUE, 0, sizeof(int), &n,
                                0, NULL, NULL);
