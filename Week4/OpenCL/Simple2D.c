...
int main(void) {
    ...
    *(local_work_size + 1)  = block_size.y;
    ...
    cl_mem array_mem_obj =
        clCreateBuffer(context, CL_MEM_WRITE_ONLY, num_bytes, NULL, &err);
    ...
    err =
        clSetKernelArg(kernel, 0, sizeof(array_mem_obj), (void*)&array_mem_obj);
    ...
    err = clEnqueueReadBuffer(queue, array_mem_obj, CL_TRUE, 0, num_bytes,
                              array, 0, NULL, NULL);
    ...
    for (int row = 0; row < num_elements_y; ++row) {
        for (int col = 0; col < num_elements_x; ++col)
            printf("%2d ", array[row * num_elements_x + col]);
        printf("\n");
    }
    printf("\n");
